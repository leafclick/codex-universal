...
| 49 | Fix | Run Clojure-helper runtime smoke fixtures with the image's pinned Babashka instead of an arbitrary host installation. | DYNAMICALLY CONFIRMED | Completed by extracting the full helper/supervisor/Unix-transport lifecycle fixture and invoking it from each available image check. The pinned `1.13.220` runtime passes all 13 network-context tests and the complete lifecycle fixture; the host-only suite passes without invoking `bb`. | M | Host launcher and synchronization validation must not depend on a runtime used only inside the product image. |
