...
legacy Landlock backend and verifies direct Landlock enforcement inside the
Docker VM because Docker Desktop and OrbStack commonly reject nested user
namespaces; the Bubblewrap-isolated persistent Clojure service and Clojure LSP
MCP integration are therefore disabled on this backend. Use
`--prefix ABSOLUTE_PATH` for a prefix other than `~/.local`, or `--dry-run` to
inspect the plan without changing the filesystem.
