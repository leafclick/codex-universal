...
| 3 | Per-lane snapshots and explicit machine handoff | Independent generations; incomplete transfer refusal; destination configuration reprovisioned/rebound; divergence preserves both copies; rollback tested | Partial: lane namespaces/locks/baselines reuse the validated snapshot engine; end-to-end second-machine acceptance remains | M-H |
