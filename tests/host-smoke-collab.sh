...
codex_host_require_capabilities path stat checksum lock atomic-replace null-sort || exit 1
T="$(codex_host_path_existing "$T")"
for c in awk git jq mktemp date od tr install cmp find sed; do command -v "$c" >/dev/null || fail "missing $c"; done
...
codex_host_atomic_replace "$T/tampered.json" "$in/$id.json"
...
bh="$(jq -j '.body' "$tmp" | codex_host_sha256_stdin)"; jq --arg h "$bh" '.body_sha256=$h' "$tmp" > "$in/$id.json"
...
mismatch_body_hash="$(printf '%s' mismatch | codex_host_sha256_stdin)"
...
[[ "$(codex_host_file_size "$body_exact")" == 65536 ]] || fail exact-body-fixture
...
for file in "$bulk_completed"/*.json; do
    digest="$(codex_host_sha256_file "$file")"
    extra_id="${file##*/}"
...
printf '%s\n' \
    ...
    '    "$CODEX_TEST_REAL_MV" -T -- "$3.replaced" "$3"' \
    'fi' \
...
prune_pre_body_hash="$(jq -j '.body' "$T/prune-pre-body.json" | codex_host_sha256_stdin)"
