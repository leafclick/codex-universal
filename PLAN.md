...
| 7 | Security issue | Preserve a fresh Bubblewrap `/dev` and bind only required NVIDIA device nodes in CUDA images. | DYNAMICALLY CONFIRMED | Implemented and argv-validated; CUDA works through approved elevation, but the active CUDA image predates this wrapper and cannot validate its device isolation. | M-H | Binding all container devices weakens device isolation. |
...
| 8 | Fix | Make the image copy of `scripts/codex-worker-observe` executable and test its mode in real images. | SOURCE CONFIRMED | Implemented in both Dockerfiles; the active CUDA image still packages mode `0644`, proving it predates this fix. | L | Image-local worker inspection otherwise fails at its documented path. |
...
| 26 | Security issue | Decide and test whether the nested-userns filter must deny `setns`; keep the outer sandbox assumptions explicit. | SOURCE CONFIRMED | Completed; filtered children now receive `EPERM` from `setns`, while outer Bubblewrap performs namespace setup before installing the filter. | M | Namespace re-entry is unnecessary for the filtered Clojure/LSP children. |
...
| 25 | Fix | Correct CUDA diagnostic process handling, accepted modes, and output capture. | SOURCE CONFIRMED | Implemented with waited bounded readers, interactive-auto bypass, NVIDIA gating, and conventional boolean modes; CUDA is functional, but the active image still contains the old wrapper. | M | Diagnostics must not race, leak helpers, or obscure command results. |
